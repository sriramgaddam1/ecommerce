...
/* Load profile photo ONLY if userId exists */
if (id) {
  const photoUrl = `http://localhost:8080/api/auth/user/${id}/photo`;
  ...
  const fetchOrders = async (id) => {
    try {
      const response = await axios.get(`http://localhost:8080/api/auth/user/${id}/orders`);
  ...
  const handleViewDetails = async (orderId) => {
    try {
      const resp = await axios.get(`http://localhost:8080/api/auth/user/${userId}/orders/${orderId}`);
  ...
  const handleReorder = async (orderId) => {
    ...
    try {
      const resp = await axios.post(`http://localhost:8080/api/auth/user/${userId}/orders/${orderId}/reorder`);
      const newOrder = resp.data;
      toast.success("Reorder placed successfully!");
      // Refresh orders and open new order details
      fetchOrders(userId);
  ...
  const handleCancelOrder = async (orderId) => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:8080/api/auth/user/${userId}/orders/${orderId}/cancel`
      );
...
const fetchUserProfile = async (id) => {
  try {
    const response = await axios.get(
      `http://localhost:8080/api/auth/user/${id}/profile`
    );
    ...
    // ✅ PHOTO IS NOT HANDLED HERE
    // Loaded directly from backend image endpoint
...
const fetchAddresses = async (id) => {
  try {
    const response = await axios.get(
      `http://localhost:8080/api/auth/user/${id}/addresses`
    );
...
const handleUpdateProfile = async () => {
  ...
  try {
    // 1️⃣ Update profile details (NO PHOTO)
    const profileData = {
    ...
    // 2️⃣ Update username locally
    localStorage.setItem("username", newUsername);
    setUsername(newUsername);

    // 3️⃣ Upload photo if selected
    if (photoFile) {
      const formData = new FormData();
      formData.append("file", photoFile);

      await axios.post(
        `http://localhost:8080/api/auth/user/${userId}/upload-photo`,
        formData,
        { headers: { "Content-Type": "multipart/form-data" } }
      );

      // 🔥 Force image reload (cache bust)
      setProfilePhoto(
        `http://localhost:8080/api/auth/user/${userId}/photo?t=${Date.now()}`
      );
  ...
  const handleSaveAddress = async () => {
    ...
    setLoading(true);
    try {
      let url = `http://localhost:8080/api/auth/user/${userId}/address`;
      let method = "POST";

      if (editingAddressId) {
        url += `/${editingAddressId}`;
        method = "PUT";
      }

      const config = { method, url, data: addressForm };
  ...
  const handleDeleteAddress = async (addressId) => {
    ...
    try {
      await axios.delete(`http://localhost:8080/api/auth/user/${userId}/address/${addressId}`);
  ...
  const handleDefaultAddress = async (addressId) => {
    try {
      await axios.put(`http://localhost:8080/api/auth/user/${userId}/address/${addressId}/default`);
  ...
  const handleSavePayment = async () => {
    ...
    setLoading(true);
    try {
      let url = `http://localhost:8080/api/auth/user/${userId}/payment-method`;
      let method = "POST";

      if (editingPaymentId) {
        // For edit, we won't use this endpoint - would need to be implemented separately
        toast.error("Payment method editing not yet fully implemented");
        setLoading(false);
        return;
      }

      const config = { method, url, data: paymentForm };
      const response = await axios(config);
  ...
  const handleDeletePayment = async (paymentId) => {
    ...
    try {
      await axios.delete(`http://localhost:8080/api/auth/user/${userId}/payment-method/${paymentId}`);
  ...
  const handleDefaultPayment = async (paymentId) => {
    try {
      await axios.put(`http://localhost:8080/api/auth/user/${userId}/payment-method/${paymentId}/default`);
